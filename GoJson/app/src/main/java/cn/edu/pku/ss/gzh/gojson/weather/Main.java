...
public class Main extends Activity {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case SET:
				Util util = new Util();
				try {
					List<Map<String, Object>> all = util.getInformation(msg.obj.toString());
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		super.setContentView(R.layout.xml_json_layout); // 设置要使用的布局管理器
	...
	private class NewThread implements Runnable {
		public void run() {
			String address = "http://apistore.baidu.com/microservice/weather?citypinyin=" + citynameEditText.getText().toString();

			HttpDownloader httpDownloader = new HttpDownloader();
			String jonString = httpDownloader.download(address);
			Message msg = Main.this.handler.obtainMessage(Main.SET, jonString);
